...
# -*- coding: utf-8 -*-
"""
CGF frozen localized-front activation and matched-baseline campaign (F).

Scientific question
-------------------
Does the physical fingerprint activate a pilot-residual localized allocation
for a fixed localized layer, after correctly retaining a simple global method
on the smooth homogeneous control E?

The manufactured benchmark is

    -Delta u = f  in (0,1)^2,
           u = 0  on the boundary,
    u(x,y) = sin(pi*x) sin(pi*y) [1 + A tanh((x-x_f)/sigma)].
...
The selector is evaluated before any allocation outcome.  Its frozen F branch
uses only the numerical pilot to detect a compact high-gradient layer.  It
selects a protected residual-adaptive allocation and explicitly records

    domain_decomposition_activated = False
    interface_enrichment_activated = True.
...
Paper-I boundary
----------------
This code addresses the F activation half of A1 and the F part of M1.  It contains no transmission
condition, overlap, coarse space, moving front, coefficient interface, or
domain-decomposition loop.  F tests allocation activation only; the final
E/S/F selector is frozen before SPE10 P.
"""
...
class Config:
    ...
    ghg_coverage_floor_fraction: float = 0.65
    alpha_green: float = 0.0
    beta_halton: float = 0.0
    gamma_gradient: float = 1.0
    ...
    # Frozen F fingerprint: homogeneous coefficient plus a compact pilot layer.
    homogeneous_contrast_max: float = 1.05
    coefficient_jump_area_max: float = 0.01
    pilot_high_frequency_fraction_max: float = 0.05
    localized_gradient_level: float = 0.65
    localized_active_fraction_max: float = 0.25
    selected_global_family: str = "RESIDUAL_ADAPTIVE"

    # Frozen admissibility and competitiveness gates.
    condition_proxy_max: float = 2.0e6
    range_violation_max: float = 2.5e-1
    conservation_relative_defect_max: float = 3.5e-1
    independent_residual_relative_max: float = 1.25
    empty_cell_fraction_max: float = 0.85
    separation_min: float = 1.0e-7
    competitive_rmse_ratio_max: float = 1.50
    competitive_front_ratio_max: float = 1.00
...
def default_campaign_dir(profile: str) -> str:
    return (
        "localized_front_smoke"
        if profile == "smoke"
        else "localized_front_activation_campaign"
    )
...
def frozen_experiment_contract(cfg: Config) -> Dict[str, Any]:
    ...
    contract: Dict[str, Any] = {
        "contract_version": "CGF-F-1.0",
        "paper_I_scope": "frozen localized-front activation control F",
        "benchmark": {
            "domain": "(0,1)^2",
            "operator": "-Delta",
            "coefficient": 1.0,
            "manufactured_state": "sin(pi*x)*sin(pi*y)*(1+0.55*tanh((x-0.58)/0.055))",
            "dirichlet_trace": 0.0,
        },
        "seeds": seeds,
        "budgets": budgets,
        ...
        "fingerprint_thresholds": {
            ...
            "localized_gradient_level": cfg.localized_gradient_level,
            "localized_active_fraction_max": cfg.localized_active_fraction_max,
        ...
        "publication_rule": (
            "F passes only if the pre-solve fingerprint selects the protected "
            "residual-adaptive family, activates finite-layer enrichment but not DDM, and "
...
FRONT_X = 0.58
FRONT_SIGMA = 0.055
FRONT_AMPLITUDE = 0.55


def _front_terms(x: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    z = (np.asarray(x) - FRONT_X) / FRONT_SIGMA
    t = np.tanh(z)
    sech2 = 1.0 - t**2
    g = 1.0 + FRONT_AMPLITUDE * t
    gp = FRONT_AMPLITUDE * sech2 / FRONT_SIGMA
    gpp = -2.0 * FRONT_AMPLITUDE * sech2 * t / FRONT_SIGMA**2
    return g, gp, gpp


def exact_u(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    g, _, _ = _front_terms(x)
    return np.sin(np.pi * np.asarray(x)) * np.sin(np.pi * np.asarray(y)) * g


def exact_ux(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    g, gp, _ = _front_terms(x)
    return np.sin(np.pi * np.asarray(y)) * (np.pi * np.cos(np.pi * np.asarray(x)) * g + np.sin(np.pi * np.asarray(x)) * gp)


def exact_uy(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    g, _, _ = _front_terms(x)
    return np.pi * np.sin(np.pi * np.asarray(x)) * np.cos(np.pi * np.asarray(y)) * g


def source_f(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    x = np.asarray(x)
    y = np.asarray(y)
    g, gp, gpp = _front_terms(x)
    sx, cx, sy = np.sin(np.pi*x), np.cos(np.pi*x), np.sin(np.pi*y)
    return 2.0*np.pi**2*sx*sy*g - 2.0*np.pi*cx*sy*gp - sx*sy*gpp
...
def physical_fingerprint(pilot: Dict[str, Any], cfg: Config) -> Dict[str, Any]:
    ...
    homogeneous_coefficient = bool(
        contrast <= cfg.homogeneous_contrast_max
        and jump_area <= cfg.coefficient_jump_area_max
    )
    grad = np.asarray(pilot["gradient"], float)
    cutoff = cfg.localized_gradient_level
    active_fraction = float(np.mean(grad >= cutoff))
    localized = bool(homogeneous_coefficient and 0.0 < active_fraction <= cfg.localized_active_fraction_max)
    selected = cfg.selected_global_family if localized else "FINGERPRINT_FAIL"
    return {
        ...
        "classified_smooth_homogeneous_coefficient": homogeneous_coefficient,
        "localized_active_fraction": active_fraction,
        "localized_active_fraction_threshold": cfg.localized_active_fraction_max,
        "classified_localized_front": localized,
        "problem_class": "FROZEN_LOCALIZED_FRONT" if localized else "FINGERPRINT_FAIL",
...
def field_metrics(
    ...
    exact_minimum, exact_maximum = float(exact.min()), float(exact.max())
    range_violation = max(0.0, exact_minimum - minimum) + max(0.0, maximum - exact_maximum)
    front_mask = np.abs(EX - FRONT_X) <= 2.5 * FRONT_SIGMA
    return {
        "full_rmse": float(np.sqrt(np.mean(error**2))),
        "full_linf": float(np.max(np.abs(error))),
        "front_collar_rmse": float(np.sqrt(np.mean(error[front_mask]**2))),
...
def allocation_gate_rows(
    ...
    for item in metrics.itertuples(index=False):
        checks = [
            (
                "G0_FINGERPRINT_LOCALIZED_FRONT",
                bool(fingerprint["classified_localized_front"]),
                "pre-solve localized-layer classification",
            ),
            (
                "G1_CONDITION",
...
def selector_gate_rows(
    ...
    if eligible.empty:
        best_rmse = best_front = best_h1 = fastest = float("nan")
        rmse_ratio = front_ratio = h1_ratio = cost_ratio = float("inf")
    else:
        best_rmse = float(eligible["full_rmse"].min())
        best_front = float(eligible["front_collar_rmse"].min())
        ...
        front_ratio = float(selected_row["front_collar_rmse"] / max(best_front, 1.0e-300))
        h1_ratio = float(
    ...
    correct_activation = bool(
        fingerprint["classified_localized_front"]
        and not fingerprint["domain_decomposition_activated"]
        and fingerprint["interface_enrichment_activated"]
        and selected_name == "RESIDUAL_ADAPTIVE"
    )
    checks = [
        (
            "S0_FROZEN_LOCALIZATION_ACTIVATION",
            correct_activation,
            "protected localized allocation selected before outcomes; DDM false",
        ),
        (
        ...
        (
            "S2_FULL_RMSE_SAFEGUARD",
            rmse_ratio <= cfg.competitive_rmse_ratio_max,
            f"selected/best eligible RMSE={rmse_ratio:.6g}",
        ),
        (
            "S2B_FRONT_COLLAR_COMPETITIVE",
            front_ratio <= cfg.competitive_front_ratio_max,
            f"selected/best eligible front-collar RMSE={front_ratio:.6g}",
        ),
        (
            "S3_H1_COMPETITIVE",
    ...
    return pd.DataFrame([
        {
            ...
            "selected_front_ratio": front_ratio,
...
def run_one(
    ...
    try:
        logger.info("Starting F run: seed=%d budget=%d", seed, budget)
        run_config = {
            "seed": seed,
            "budget": budget,
            "profile": cfg.profile,
        ...
        logger.info("Completed F run: seed=%d budget=%d", seed, budget)
        close_logger(logger)
...
def campaign_summary_text(
    ...
    decision = (
        "F ACTIVATION-BRANCH PASS"
        if scientific and len(final) == len(seeds) * len(budgets) and final["passed"].all()
        else (
            "F ACTIVATION-BRANCH FAIL/DEFER"
            if scientific
            else "NON-SCIENTIFIC SMOKE: NO F/A1 ADJUDICATION"
        )
    )
    return "\n".join([
        "CGF PAPER-I FROZEN LOCALIZED-FRONT ACTIVATION SUMMARY",
        "=" * 72,
        ...
        decision,
        "",
        "Interpretation discipline: F tests static localized allocation only. It contains",
        "no DDM, moving interface, or transmission law. The final E/S/F selector",
        "is frozen only after this independent F experiment is complete.",
        "",
    ])
...
def aggregate_campaign(
    ...
    plot_accuracy(allocation, campaign_dir / "localized_front_accuracy_refinement.png")
    plot_residual_condition(
        allocation, campaign_dir / "localized_front_residual_condition.png"
    )
    plot_node_diagnostics(
        nodes, campaign_dir / "localized_front_node_diagnostics.png"
    )
    plot_selector(
        selector, campaign_dir / "localized_front_selector_competitiveness.png"
    )
    plot_allocation_example(
        campaign_dir, budgets, campaign_dir / "localized_front_allocations.png"
    )

    figure_sources = [
        ("localized_front_accuracy_refinement.png", "campaign_allocation_metrics.csv", "Results: F accuracy"),
        ("localized_front_residual_condition.png", "campaign_allocation_metrics.csv", "Results: F admissibility"),
        ("localized_front_node_diagnostics.png", "campaign_node_metrics.csv", "Results: F allocation geometry"),
        ("localized_front_selector_competitiveness.png", "campaign_selector_gate_matrix.csv", "Algorithm: localization activation"),
        ("localized_front_allocations.png", "nodes_* per-run CSV", "Methods: matched allocations"),
    ]
    provenance_rows = []
    ...
    gap_rows = [
        ("M1-F", "Frozen localized-front benchmark definition", "frozen_experiment_contract.json", "Model Problems"),
        ("A1-F", "Executable F activation branch", "campaign_selector_gate_matrix.csv", "Main Results"),
        ("F1-F", "F paper-facing figures", "figure_provenance.csv", "Figures"),
        ("X1-F", "Replay provenance", "manifests/configs/state arrays", "Data/Code"),
    ]
    pd.DataFrame(
        gap_rows,
    ...
    (campaign_dir / "activation_decision_summary.txt").write_text(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the CGF frozen localized-front activation campaign"
    )
...
def main() -> int:
    ...
    if not fingerprint["classified_localized_front"]:
        raise RuntimeError(
            "The declared F benchmark did not pass its pre-solve fingerprint. "
            "Do not inspect allocation outcomes or retune thresholds."
        )
    if fingerprint["selected_architecture"] != cfg.selected_global_family:
        raise RuntimeError("pre-solve selector did not activate the frozen localized family.")
    ...
    write_manifest(campaign_dir, "campaign_manifest_sha256.csv")
    print((campaign_dir / "activation_decision_summary.txt").read_text(encoding="utf-8"))
